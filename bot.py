        ...
        if port_str:
            ...
            def _run_discord() -> None:
                if not DISCORD_TOKEN:
                    log.warning("DISCORD_TOKEN is not set — HTTP API only (/health, /login). Set token in Railway to run the bot.")
                    return
                try:
                    log.info("Starting Discord bot (background thread)...")
                    bot.run(DISCORD_TOKEN)
                except Exception as e:
                    log.critical(f"Discord bot exited: {e}", exc_info=True)

            threading.Thread(target=_run_discord, daemon=True).start()
            port = int(port_str)
            log.info("Starting HTTP API for loader (port %s) — same process as bot on Railway", port)
            uvicorn.run(
                "auth_api:app",
                host="0.0.0.0",
                port=port,
