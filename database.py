...
class AuthDatabase:
    ...
    def _run_migrations(self):
        ...
        try:
            # Check if keys table has new columns
            ...
            if 'hwid_locked' not in columns:
                log.info("Adding hwid_locked column to keys table")
                self.cursor.execute('ALTER TABLE keys ADD COLUMN hwid_locked BOOLEAN DEFAULT 0')
            
            if 'hwid' not in columns:
                log.info("Adding hwid column to keys table")
                self.cursor.execute('ALTER TABLE keys ADD COLUMN hwid TEXT')
            
            if 'duration_days' not in columns:
                log.info("Adding duration_days column to keys table")
                self.cursor.execute('ALTER TABLE keys ADD COLUMN duration_days INTEGER')
            
            if 'duration_type' not in columns:
                log.info("Adding duration_type column to keys table")
                self.cursor.execute('ALTER TABLE keys ADD COLUMN duration_type TEXT DEFAULT "lifetime"')
    ...
    def transaction(self):
        ...
        try:
            ...
            self.conn.rollback()
            log.debug(f"Transaction rolled back (integrity): {e}")
            raise
    ...
    def bulk_generate_keys(self, product_name: str, count: int, expires_days: int = None, 
                        hwid_locked: bool = False, hwid: str = None,
                        duration_type: str = 'lifetime') -> List[str]:
        ...
        try:
            ...
            for _ in range(count):
                ...
                if self.add_key(key, product_name, expires_at, hwid_locked, hwid, expires_days, duration_type):
                    keys.append(key)
    ...
    def add_key(self, key: str, product_name: str, expires_at: str = None, 
                hwid_locked: bool = False, hwid: str = None,
                duration_days: int = None, duration_type: str = 'lifetime') -> bool:
        """Add a new license key to database"""
        try:
            with self.transaction():
                self.cursor.execute(
                    'INSERT INTO keys (key, product_name, expires_at, hwid_locked, hwid, duration_days, duration_type) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    (key, product_name, expires_at, hwid_locked, hwid, duration_days, duration_type)
                )
            log.info(f"Added key for product {product_name} (duration: {duration_type})")
            return True
        except sqlite3.IntegrityError:
    ...
    def validate_key(self, key: str, user_hwid: str = None) -> tuple[bool, str, dict]:
        """Validate a license key and return product name and key info if valid"""
        try:
            self.cursor.execute('''
                SELECT product_name, is_used, expires_at, is_expired, hwid_locked, hwid, duration_days, duration_type 
                FROM keys 
                WHERE key = ?
            ''', (key,))
            ...
            key_info = {
                ...
                'expires_at': result[2],
                'is_expired': result[3],
                'hwid_locked': result[4],
                'hwid': result[5],
                'duration_days': result[6],
                'duration_type': result[7]
            }
            
            # Check if key is already used
            if key_info['is_used']:
                return False, key_info['product_name'], key_info
            ...
            if key_info['expires_at']:
                from datetime import datetime
                if datetime.now().strftime('%Y-%m-%d %H:%M:%S') > key_info['expires_at']:
                    # Mark as expired
                    self.mark_key_expired(key)
                    return False, key_info['product_name'], key_info
            
            # HWID lock: only compare after first bind (hwid NULL/empty = not bound yet)
            bound_hwid = key_info.get('hwid')
            if key_info['hwid_locked'] and user_hwid and bound_hwid:
                if bound_hwid != user_hwid:
                    log.warning(f"HWID mismatch for key {key[:8]}... expected: {bound_hwid}, got: {user_hwid}")
                    return False, key_info['product_name'], key_info
            
            return True, key_info['product_name'], key_info
        except sqlite3.Error as e:
            log.error(f"Failed to validate key: {e}", exc_info=True)
            return False, "", {}
    
    def mark_key_used_by_hwid(self, key: str, user_id: int, user_hwid: str) -> bool:
        """Mark key as used and lock to HWID"""
        try:
            with self.transaction():
                self.cursor.execute(
                    'UPDATE keys SET is_used = 1, used_by = ?, used_at = CURRENT_TIMESTAMP, hwid = ? WHERE key = ?',
                    (user_id, user_hwid, key)
                )
            log.info(f"Marked key {key[:8]}... as used by user {user_id} with HWID {user_hwid}")
            return True
        except sqlite3.Error as e:
            log.error(f"Failed to mark key as used: {e}", exc_info=True)
            return False
